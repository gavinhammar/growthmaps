<template>
    <v-flex justify-center xs4 md4 lg3>
    <v-flex class="white elevation-4 mx-auto pa-0 ma-3"  >
        <v-container v-if="objective.type=='vision'" >
             <h4>Vision</h4>
            <div> {{ objective.title === ""? "What's your company's vision?" : objective.title }}</div>
            <v-btn small color="primary">Set Vision</v-btn>
        </v-container>
        <v-container v-if="objective.type=='objective'">
            ...
            <v-btn small color="primary">Set Objective</v-btn>
            
        </v-container>
    </v-flex>
    <v-flex justify-center class="mx-auto " xs4 v-if="objective.type!='vision'" >
...
export default {
  name: 'Objective',
  props: ['type', 'objective', 'index', 'count'],
  components: {
      PlusKeyResult
  },
  data: function() {
